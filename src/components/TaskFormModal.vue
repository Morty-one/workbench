<!--
  ...
  Props:
    ...
    autoGenerated- 该任务是否由「预设任务」规则自动生成（编辑模式删除时，确认文案要额外提示
                   「删掉明天还会再生成」，否则用户会以为删了就没了）

  Emits:
    update:show  - 关闭时通知父组件
    submit       - 提交时携带完整表单数据（深拷贝）
    remove       - 点「删除任务」时携带任务 id，由父组件落库删除并负责关窗/刷新
-->
<script setup>
...
const emit = defineEmits(['update:show', 'submit', 'remove'])
 ...
 * 删除任务（仅编辑模式，由父组件落库）。
 *
 * 确认文案里的标题用 initialData.title（**库里那条记录的标题**）而不是 form.title：
 * 表单可能已经被改过，而要被删掉的是库里那条，用原标题才诚实。
 *
 * autoGenerated 时额外提示「明天还会再生成」：删任务**不碰预设规则**，
 * 这是架构必然（规则每天照常生成）。不提示的话用户会以为「删了怎么又冒出来」。
 */
async function localRemove() {
  if (!props.editingId) return
  const name = String((props.initialData || {}).title || form.title || '').trim() || '（未命名任务）'
  const ok = await appConfirm({
    title: '删除任务「' + name + '」',
    message: props.autoGenerated
      ? '这条是「预设任务」自动生成的，删掉后明天还会按规则再生成；想让它以后不再出现，请到设置中心把对应规则停用。'
      : '',
    warn: '此操作不可恢复。',
    danger: true,
    okText: '删除任务'
...
<template>
  <div v-if="show" class="modal-mask" @click.self="close">
    <div class="modal">
      ...
      <!-- 底部：编辑模式才有「删除任务」，靠 CSS margin-right:auto 顶到最左，
           与右半边的「取消 / 保存」拉开距离（防想点取消却点到删除） -->
      <div class="row modal-foot" style="justify-content: flex-end; margin-top: 14px">
        <button v-if="editingId" class="ghost danger modal-del" @click="localRemove">删除任务</button>
...
<style scoped>
/* 底部按钮行：用 margin-right:auto 把「删除任务」顶到最左。
   不用 flex:1 的空 span 占位 —— 窄屏换行时 spacer 会把行搞乱，margin-right:auto 更稳。
   .row 本身没有 flex-wrap，三个按钮在窄屏可能挤爆 ⇒ 这里补上换行（换行后仍右对齐）。 */
.modal-foot {
  flex-wrap: wrap;
}
.modal-foot .modal-del {
  margin-right: auto;
}
.grid2 {
  display: grid;
